...
# def RISAngle_imshow_det_rbboxes(img,
#                                 bboxes=None,
#                                 labels=None,
#                                 contours=None,
#                                 segms=None,
...
#                                 wait_time=0,
#                                 out_file=None):
#     '''Draw rbboxes, contour_masks and class labels (with scores) on an image.
#     Args:
#         img (str | ndarray): The image to be displayed.
#         bboxes (ndarray): Bounding boxes (with scores), shaped (n, 5) or
#             (n, 6).
#         labels (ndarray): Labels of bboxes.
#         contours (ndarray): contour poly, shaped (n, num_points), 默认情况是32
#         segms (ndarray | None): Masks, shaped (n,h,w) or None.
#         class_names (list[str]): Names of each classes.
#         score_thr (float): Minimum score of bboxes to be shown. Default: 0.
#         bbox_color (list[tuple] | tuple | str | None): Colors of bbox lines.
...
#     assert segms is not None or bboxes is not None, \
#         'segms and bboxes should not be None at the same time.'
#     assert contours is None or contours.shape[0] == labels.shape[0], \
#         'contours.shape[0] and labels.shape[0] should have the same length.'
#     img = mmcv.imread(img).astype(np.uint8)
#
#     if score_thr > 0:
#         assert bboxes is not None and bboxes.shape[1] == 6
...
#
#
# def RISDistance_imshow_det_rbboxes(img,
#                                 bboxes=None,
#                                 labels=None,
#                                 img_poly=None,
#                                 can_poly=None,
